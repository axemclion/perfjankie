...
module.exports = function(config) {
	...
	return uploadViews(db, log).then(function() {
		var browserPerf = require('browser-perf');
		var apiDocs = new browserPerf.docs();
		apiDocs.metrics.type = 'metadata';
		return Q.ninvoke(db, 'get', 'metrics-list').then(function(doc) {
			apiDocs.metrics._rev = doc[0]._rev;
			return Q.ninvoke(db, 'insert', apiDocs.metrics, 'metrics-list');
		}, function(err) {
			return Q.ninvoke(db, 'insert', apiDocs.metrics, 'metrics-list');
		});
	});
